...
from project.utils.transorms import get_preprocessing
from project.utils.const import (
    INPUT_FOLDER,
    MASK_FOLDER,
...
def sample_from_ddpm_pretrain_cond(
    ddpm: DDPM,
    cond: torch.Tensor,
    img: torch.Tensor,
    device: torch.device,
...
def sample_from_ddpm(
    ...
    #     cond_tmp = torch.cat((cond_tmp, cond_tmp), dim=0)
    ...
    # timesteps = reversed(range(0, num_timesteps))
    iterator = tqdm(reversed(range(0, num_timesteps)), total=num_timesteps, leave=True, disable=False)
    
    for i, step in enumerate(iterator):
        context, img = ddpm.p_sample(
            img,
            conditioning,  # 进入 p sample函数
            torch.full((1,), step, device=device, dtype=torch.long),
            torch.full((1,), fill_value=num_timesteps - i - 1, device=device, dtype=torch.long),
            clip_denoised=ddpm.clip_denoised,
            cond_fn=cond_fn
        )
        if cond_fn and context["g_apply"]:
            loss_val = context["g_loss"]
            desc = f"Spaced Sampler With Guidance, Loss: {loss_val:.6f}"
        else:
            desc = "Spaced Sampler"

        iterator.set_description(desc)
    brain_img = decoder(img)
    return brain_img
