...
from models.ddpm_v2_conditioned_cond_pretrain import DDPM
import numpy as np
import torch
...
from project.utils.utils import (
    sampling_from_ddim,
    sample_from_ddpm_pretrain_cond,
)
...
def main(args) -> None:
    ...
    diffusion = DDPM(unet_config) # describe the graph shape


    sd = torch.load('/data1/baihy/denoise/diffusion.pt', map_location="cpu")

    diffusion.load_state_dict(sd)
    ...
    opt_cond = torch.optim.AdamW(cond_pretrain.parameters(), lr=0.001)
    import torch.optim.lr_scheduler as lr_scheduler
    scheduler_cond = lr_scheduler.CosineAnnealingLR(opt_cond, T_max=400, eta_min=1e-9)

    decoder.eval().to(device)
    
    cond_pretrain, opt_cond, loader, val_loader, scheduler_cond = \
        accelerator.prepare(cond_pretrain, opt_cond, loader, val_loader, scheduler_cond)
    ...
    while global_step < max_steps:
        ...
        for gt, _ in loader:
            ...
            with torch.no_grad():  # 训练的时候 这下面的三个 都是相当于测试 全部冻住了（蓝色），只需要向前forward
                ...
                # if accelerator.is_local_main_process:
                #     print(z_gt.shape)
                cond, latent_variable = setup_noise_inputs(device=device, img = z_gt, batchsize = 1, c_txt = c_txt) 
            ...
            if global_step % cfg.train.image_every == 0 or global_step == 1:
                ...
                with torch.no_grad():
                    synth_img = sample_from_ddpm_pretrain_cond(
                    ddpm = diffusion,
                    decoder = decoder,
                    img=latent_variable,
                    device=accelerator.device,
