...
def main(args) -> None:
    ...
    while global_step < max_steps:
        ...
        for gt, lq, _ in loader:
            ...
            pred = swinir(lq)
            mse_loss = F.mse_loss(input=pred, target=gt, reduction="mean") * 10
            ...
            step_mse_loss.append(mse_loss)
            step_ssim_loss.append(ssim_loss)
            epoch_loss.append(loss.item())
            pbar.update(1)
            pbar.set_description(f"Epoch: {epoch:04d}, Global Step: {global_step:07d}, Loss: {loss.item():.6f}, MSE Loss: {mse_loss.item():.6f}, SSIM Loss: {ssim_loss.item():.6f}")

            # Log loss values:
            if global_step % cfg.train.log_every == 0:
                # Gather values from all processes
                avg_mse_loss = accelerator.gather(torch.tensor(step_ssim_loss, device=device).unsqueeze(0)).mean().item()
                avg_ssim_loss = accelerator.gather(torch.tensor(step_mse_loss, device=device).unsqueeze(0)).mean().item()
                ...
                if accelerator.is_local_main_process:
                    ...
                    writer.add_scalar("train/ssim_loss_step", avg_ssim_loss, global_step)
            ...
            if global_step % cfg.train.val_every == 0:
                ...
                for val_gt, val_lq, _ in val_loader:
                    ...
                    val_gt = rearrange(val_gt , "b h w c -> b c h w").squeeze(1).contiguous().float().to(device)
                    val_lq = rearrange(val_lq, "b h w c -> b c h w").squeeze(1).contiguous().float().to(device)
                    with torch.no_grad():
                        # forward
                        val_pred = swinir(val_lq.unsqueeze(1))  # SwinIR预计一个通道维度，对于灰度图，需要unsqueeze(1)添加通道维度
                        # compute metrics (loss, lpips, psnr)
                        val_loss.append(F.mse_loss(input=val_pred, target=val_gt.unsqueeze(1), reduction="sum").item())  # 对于灰度图也需要添加unsqueeze(1)
                        val_lpips.append(lpips_model(val_pred, val_gt.unsqueeze(1), normalize=True).mean().item())
                        val_psnr.append(calculate_psnr_pt(val_pred, val_gt.unsqueeze(1), crop_border=0).mean().item())
                    val_pbar.update(1)
                val_pbar.close()
